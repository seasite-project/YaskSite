...
void perfModel::getBytePerCycles()
{
    ...
    readTable(bw_file, bw_table, 1, TH_P_SOCK, 2, 2);
    delete[] bw_file;
...
std::vector<double> perfModel::getDataContrib(char* cache_str, blockDetails* opt, int temporalStoreMode)
{
    ...
    std::vector<double> assocOh = simulateCache_assoc(currCache);
    ...
    {
        double totalSpatialContrib = (rest_contrib+1.0+spatialOh[0]+assocOh[0]+prefetch_oh_per_LUP);/*TODO compute per region*/
        ...
        ECM_prefetch[currCache.hierarchy] = prefetch_oh_per_LUP*currCache.bytePerWord/static_cast<double>(stencil->data->dt);
    ...
    {
        ECM_latency[currCache.hierarchy] = (totalGrids)*latency_oh_per_LUP;
        double prefetch_oh = totalGrids*prefetch_oh_per_LUP;
        double totalData = rest_contrib+1.0+spatialOh[0]+assocOh[0]+prefetch_oh;
        ECM_prefetch[currCache.hierarchy] = prefetch_oh*currCache.bytePerWord;
        ECM_boundary[currCache.hierarchy] = spatialOh[0]*currCache.bytePerWord;
    ...
    else if(currCache.hierarchy > opt->spatialIBC->hierarchy || (stencil->dim == 2))
    {
        ...
        ECM_latency[currCache.hierarchy] = (stencil_contrib_outer + stencil_centre + totalGrids - 1)*latency_oh_per_LUP;
        double prefetch_oh = (stencil_contrib_outer + stencil_centre + totalGrids - 1)*prefetch_oh_per_LUP;
        double totalData = stencil_contrib_outer + stencil_centre + rest_contrib + spatialOh[1] + assocOh[1] + prefetch_oh;
        ECM_prefetch[currCache.hierarchy] = prefetch_oh*currCache.bytePerWord;
        ECM_boundary[currCache.hierarchy] = spatialOh[1]*currCache.bytePerWord;
        printf("assoc Oh = %f\n", assocOh[1]);
    ...
    else
    {
        ...
        double totalData = stencil_contrib_outer + stencil_contrib_inner + stencil_centre + rest_contrib + spatialOh[2] + assocOh[2] + prefetch_oh;
        return {totalData, ((totalData-numWriteGrids)/(double)numWriteGrids)};
    }
}
...
double perfModel::getPrefetchEffects(cache_info currCache)
{
    ...
    double boundary_prefetched = std::min(2*stencil->data->radius, currCache.prefetch_cl*8)/dz; //due to lhs
    double reuseFactor = 0;
    //If only one block in z direction prefetch is reused, only extra prefetching is
    //at boundary
    if( num_block_z == 1 )
    {
        reuseFactor = 1 - boundary_prefetched;
    }
    else if(currCache.hierarchy != 0)
    {
        ...
        if(upperCache.shared == true)
        {
            ...
            else if(2*bz*actualThreads < upperCache.words)
            {
                ...
                reuseFactor = (1- (1/(double)stencil->data->nthreads) - boundary_prefetched);
            }
        }
    }
...
//traffic
std::vector<double> perfModel::simulateCache_assoc(cache_info currCache)
{
...
//    temporalOh = addBlockBoundaryEffects(true);
   ...
   //L3->L2
    ...
    int bw_idx = (int)(std::round(mem_l3_data[1]))-1;
