...
#USAGE: ./bench_scan.sh LD_ST_bench_DIR COMPILER INSTR MIN_LD MAX_LD
...
stable_freq=1 #$(taskset -c 0-0 $tool_dir/bw_bench/cpuFreqCheck.sh)
...
#    printf "%10s %6s %12s\n" "Size(MB)" "LD:ST" "BW(MB/s)" > $out_file
    ...
    output=$($ld_st_dir/LDST-bench INTEL_ASM:$instr $compiler $nthread $min_ld 1 $size -c E:N:$nthread:1:$threads_per_core)
...
#    printf "%10s %6s %12s\n" $size $min_ld $bw >> $out_file
    ...
    for (( ld=$(($min_ld+1)); ld<=$max_ld; ld++ )); do
        ...
        output=$($ld_st_dir/LDST-bench INTEL_ASM:$instr $compiler $nthread $ld 1 $size -c E:N:$nthread:1:$threads_per_core)
        bw=$(echo $output | grep -o -P "(?<=Measured Bandwidth with WA \(RFO\) =).*(?=MB/s)")
        bw_gbs=$(echo "$bw/1000.0" | bc -l)
        bw_gbs=$(printf '%.*f\n' 2 $bw_gbs)
...
#        printf "%10s %6s %12s\n" $size $ld $bw >> $out_file
    ...
    output=$($ld_st_dir/LDST-bench INTEL_ASM:$instr $compiler $nthread 1 0 $size -c S0:0-$((nthread-1)))
    bw=$(echo $output | grep -o -P "(?<=Measured Bandwidth with WA \(RFO\) =).*(?=MB/s)")
    bw_gbs=$(echo "$bw/1000.0" | bc -l)
    bw_gbs=$(printf '%.*f\n' 2 $bw_gbs)
    bw_out="$bw_out, $bw_gbs GB/s"
