		...
		else if(isalpha(*str))
		{
			...
			if(tokenCnt != 0 && (tokenArray[tokenCnt - 1].type == NUMBER || tokenArray[tokenCnt - 1].type == RIGHT_PARENTHESIS))
			{
				...
				if(pos != -1)
				{
					...
					tokenArray[tokenCnt].data.operator_.arity = 2;
					tokenArray[tokenCnt].data.operator_.baseData = operators[pos];
					tokenCnt++;
				}
				else
				{
		...
		else
		{
			switch(*str)
			{
				...
				default:
					...
					{
						tokenArray[tokenCnt].type = OPERATOR;
						tokenArray[tokenCnt].data.operator_.arity = arityMode;
						tokenArray[tokenCnt].data.operator_.baseData = operators[pos];
						tokenCnt++;
					}
					else
...
void shuntingYard(queue *outputQueue, token *tokenArray, int tokenCnt) {
	...
	for(i = 0; i < tokenCnt; i++)
	{
		...
		{
			if(tokenArray[i].data.operator_.arity != 1)
			{
				while(operationStack != NULL && operationStack->data.type == OPERATOR && tokenArray[i].data.operator_.baseData.precedence <= operationStack->data.data.operator_.baseData.precedence)
				{
					pop(&operationStack, &popped);
					add(outputQueue, popped);
				}
...
double evaluate(queue *outputQueue) {
	...
	while(get(outputQueue, &popped) == OK)
	{
		...
		{
			token arg1, arg2;
			switch(popped.data.operator_.arity)
			{
				case 1:
					pop(&evaluationStack, &arg1);
					result.data.number = popped.data.operator_.baseData.function(0, arg1.data.number);
					break;
				case 2:
					pop(&evaluationStack, &arg1);
					pop(&evaluationStack, &arg2);
					result.data.number = popped.data.operator_.baseData.function(arg2.data.number, arg1.data.number);
					break;
			}
