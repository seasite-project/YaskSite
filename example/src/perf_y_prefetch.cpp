...
void main(int argc, char** argv)
{
    ...
    for(int dim=start_dim; dim<end_dim; dim+=inc_dim)
    {
        ...
        if(dimension == 3)
        {
            ...
            dim_x = 101;//static_cast<int>((200.0*L3_size)/((double)(2.0*8.0*dim*dim)));
            dim_y = 50*4+1;
           // dim_z = 1024*4 - cannot be blocked in L2 & L1; however small you make the block size due to thrashing, all mapped to same set
            dim_z = 96; //1032*4 - min of 5 words extra for bz>16; here till bz=496*8 5 words
        ...
        {
            dim_y = 1000;//static_cast<int>((200.0*L3_size)/((double)(2.0*8.0*dim)));
            dim_z = 1000;//dim;
        }
        else
        {
        ...
        if(opt_bool[1])
        {
           ...
           // stencil_1->setBlock(1500,512,32);
            if(dimension==2)
              stencil_1->setBlock(1000,1000,1);
            else
             stencil_1->setBlock(96,201,101);
